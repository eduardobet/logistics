...
{!! Form::hidden("mailers[{$key}][mid]", $mailer->id ) !!}

<div class="row det-row"{!! $mailer->id && request('id') == $mailer->id ? " style='border: red solid 1px;'" : null !!}>
    <div class="col-lg-5">
        ...
        <div class="row">
            ...
            <div class="col-1">
                <div class="form-group mg-t-30-force">
                    <button class="btn btn-sm btn-outline-danger rem-row" type="button" data-id="{{ $mailer->id ? $mailer->id : ':id:' }}" data-del-url="{{ route('tenant.mailer.destroy', $tenant->domain) }}" data-params='{"id" : "{{$mailer->id}}", "client_id" :"{{$mailer->client_id}}" }'>
