    ...
    <div class="col-lg-12">
        <div class="form-group">
            ...
            @foreach ($userBranches as $uBranch)
                <option value="{{ $uBranch->id }}" data-code="{{ $uBranch->code }}" data-dcomission="{{ $uBranch->direct_comission }}" data-should-invoice="{{ $uBranch->should_invoice }}" {{ isset($warehouse) && $warehouse->branch_from == $uBranch->id ? " selected": null }}>
                    {{ $uBranch->name }}
                </option>
            @endforeach
            </select>
...
<div class="row">
    ...
    <div class="col-lg-4">
        <div class="form-group">
            ...
            @foreach ($mailers as $mailer)
                <option value="{{ $mailer->id }}" data-vol_price="{{ $mailer->vol_price }}" data-real_price="{{ $mailer->real_price }}" {{ isset($warehouse) && $warehouse->mailer_id == $mailer->id ? " selected": null }}>
                    {{ $mailer->name }}
                </option>
            @endforeach
            </select>
    ...
    <div class="col-lg-4">
        <div class="form-group">
            ...
            <select name="branch_to" id="branch_to2" class="form-control" disabled>
            <option value="">---</option>
            @foreach ($branches as $aBranch)
                <option value="{{ $aBranch->id }}" {{ isset($warehouse) && $warehouse->branch_to == $aBranch->id ? " selected": null }}>
                    {{ $aBranch->name }}
                </option>
            @endforeach
            </select>
...
<div class="row">
    <div class="col-lg-6">
        <div class="form-group mg-b-10-force">
            <label class="form-control-label">{{ __('Tracking numbers') }} <span id="qty-dsp">(0)</span>: <span class="tx-danger">*</span></label>
            {!! Form::textarea('trackings', null, ['class' => 'form-control', 'required' => 'required', 'rows' => 4, ]) !!}
        </div>
    </div>
    <div class="col-lg-6">
        <div class="form-group mg-b-10-force">
